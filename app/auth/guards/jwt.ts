  ...
  implements GuardContract<UserProvider[typeof symbols.PROVIDER_REAL_USER]>
{
  ...
  async generate(
    ...
    const refresh_token = jwt.sign({ userId: providerUser.getId() }, this.#options.refresh_token_secret, {
      algorithm: 'HS512',
      expiresIn: '2 days'
    })
  ...
  async verifyRefreshToken(refresh_token: string) {
    ...
    * Verify token
    */
    const payload = jwt.verify(refresh_token, this.#options.refresh_token_secret)
    ...
    //   expiresIn: '12h'
    // })
    // const refresh_token = jwt.sign({ userId: providerUser.getId() }, this.#options.refresh_token, {
    //   algorithm: 'HS512',
    //   expiresIn: '2 days'
    // })
